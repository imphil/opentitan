...
def parse_fpv_summary(str_buffer):
    ...
    fpv_summary["pass_rate"] = format_percentage(
        fpv_summary["proven"], fpv_summary["cex"] + fpv_summary["undetermined"]
    )
    fpv_summary["cov_rate"] = format_percentage(
        fpv_summary["covered"], fpv_summary["unreachable"]
    )
...
def get_cov_results(logpath, dut_name):
    ...
    try:
        with Path(logpath).open() as f:
            ...
            pattern_match = (
                r"\s*\|(\d*.\d\d)%\s\(\d*\/\d*\)"  # extract percentage in cov_pattern
            )
            coverage_patterns = [
                (
                    "stimuli",
                    r"^\|" + dut_name + pattern_match + cov_pattern + cov_pattern,
                ),
                ("coi", r"^\|" + dut_name + cov_pattern + pattern_match + cov_pattern),
                (
                    "proof",
                    r"^\|" + dut_name + cov_pattern + cov_pattern + pattern_match,
                ),
            ]
            ...
            for key, item in cov_results.items():
                if len(item) == 1:
                    ...
                    cov_results[key] = "N/A"
                    log.error(
                        "Parse %s coverage error. Expect one matching value, get %s",
                        key,
                        item,
                    )
            return cov_results
...
def main():
    parser = argparse.ArgumentParser(
        description="""This script parses the JasperGold FPV log to extract below information.
    ...
    parser.add_argument(
        ...
        default="fpv.log",
        help=(
            "FPV log file path. Defaults to `fpv.log` "
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Enable parsing coverage data. " "By default, coverage parsing is disabled."
        ),
    )
