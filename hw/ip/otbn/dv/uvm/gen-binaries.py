...
def take_env_line(dst: Dict[str, str], path: str, line_number: int, line: str) -> None:
    ...
    if len(parts) != 2:
        raise RuntimeError(
            "{}:{}: No equals sign in line {!r}.".format(path, line_number, line)
        )
...
def read_toolchain(obj_dir_arg: Optional[str], otbn_dir: str) -> Toolchain:
    ...
    if obj_dir_arg is not None:
        ...
        if obj_dir_env is not None:
            obj_dir = obj_dir_env
            source = (
                "inferred from OBJ_DIR environment variable; "
            ...
            source = "inferred from script location; " "have you run meson_init.sh?"
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to read .env file at {!r} "
            "(at a path {}): {}.".format(env_path, source, ose)
        ) from None
...
def main() -> int:
    ...
    parser.add_argument(
        "--count", type=read_positive, default=10, help="Number of binaries to generate"
    )
    parser.add_argument(
        "--obj-dir",
        help=(
            "Object directory configured with Meson (used "
...
def write_ninja(
    handle: TextIO, rig_count: int, start_seed: int, toolchain: Toolchain, otbn_dir: str
) -> None:
