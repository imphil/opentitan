...
def read_word(arg_name: str, word_data: str) -> int:
    ...
    if value < 0 or value >> 32:
        raise ValueError(
            "<{}> argument is {!r}: not representable as a u32.".format(
                arg_name, word_data
            )
        )
...
def on_start(sim: OTBNSim, args: List[str]) -> None:
    ...
    if len(args) != 1:
        raise ValueError("start expects exactly 1 argument. Got {}.".format(args))

    addr = read_word("addr", args[0])
    if addr & 3:
        raise ValueError(
            "start address must be word-aligned. Got {:#08x}.".format(addr)
        )
...
def on_load_elf(sim: OTBNSim, args: List[str]) -> None:
    ...
    if len(args) != 1:
        raise ValueError("load_elf expects exactly 1 argument. Got {}.".format(args))
    path = args[0]
...
def on_load_d(sim: OTBNSim, args: List[str]) -> None:
    ...
    if len(args) != 1:
        raise ValueError("load_d expects exactly 1 argument. Got {}.".format(args))
    path = args[0]
...
def on_load_i(sim: OTBNSim, args: List[str]) -> None:
    ...
    if len(args) != 1:
        raise ValueError("load_i expects exactly 1 argument. Got {}.".format(args))
    path = args[0]
...
def on_dump_d(sim: OTBNSim, args: List[str]) -> None:
    ...
    if len(args) != 1:
        raise ValueError("dump_d expects exactly 1 argument. Got {}.".format(args))
    path = args[0]
...
def on_print_regs(sim: OTBNSim, args: List[str]) -> None:
    ...
    if len(args):
        raise ValueError("print_regs expects zero arguments. Got {}.".format(args))
...
def on_print_call_stack(sim: OTBNSim, args: List[str]) -> None:
    ...
    if len(args):
        raise ValueError(
            "print_call_stack expects zero arguments. Got {}.".format(args)
        )
