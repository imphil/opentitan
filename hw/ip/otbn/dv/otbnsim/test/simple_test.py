...
def find_simple_tests() -> List[Tuple[str, str]]:
    ...
    for subdir, _, files in os.walk(root):
        ...
        for basename, asm_file in asm_files.items():
            ...
            if exp_file is None:
                raise RuntimeError(
                    "In the directory {!r}, there is {}, but "
                    "no {}.exp, which should contain expected "
                    "values.".format(dirname, asm_file, basename)
                )

            ret.append(
                (os.path.join(dirname, asm_file), os.path.join(dirname, exp_file))
            )
        ...
        for basename, exp_file in exp_files.items():
            if basename not in asm_files:
                raise RuntimeError(
                    ...
                    "that generates the expected values.".format(
                        dirname, exp_file, basename
                    )
                )
...
def get_reg_dump(stdout: str) -> Dict[str, int]:
    ...
    for line in stdout.split("\n"):
        ...
        if not m:
            raise RuntimeError(
                "Failed to parse line after PRINT_REGS ({!r}).".format(line)
            )
...
def get_reg_expected(exp_path: str) -> Dict[str, int]:
    ...
    with open(exp_path) as exp_file:
        for idx, line in enumerate(exp_file):
            ...
            if m is None:
                raise RuntimeError(
                    "{}:{}: Bad format for line in expected.txt.".format(
                        exp_path, idx + 1
                    )
                )
            ret[m.group(1)] = int(m.group(2), 0)
...
def test_count(tmpdir: py.path.local, asm_file: str, expected_file: str) -> None:
