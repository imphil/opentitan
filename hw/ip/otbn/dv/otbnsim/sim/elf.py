...
def _get_elf_segments(
    path: str, imem_desc: _MemDesc, dmem_desc: _MemDesc
) -> Tuple[_SegList, _SegList]:
    ...
    with open(path, "rb") as handle:
        ...
        for segment in elf_file.iter_segments():
            ...
            if imem_lma <= seg_lma <= imem_top:
                if seg_top > imem_top:
                    raise RuntimeError(
                        "Segment has LMA range {:#x}..{:#x}, "
                        "which intersects the IMEM range "
                        "({:#x}..{:#x}), but is not fully "
                        "contained in it.".format(seg_lma, seg_top, imem_lma, imem_top)
                    )
            ...
            if dmem_lma <= seg_lma <= dmem_top:
                if seg_top > dmem_top:
                    raise RuntimeError(
                        ...
                        "({:#x}..{:#x}), but is not fully "
                        "contained in it.".format(seg_lma, seg_top, dmem_lma, dmem_top)
                    )
            ...
            # either IMEM or DMEM.
            raise RuntimeError(
                "Segment has LMA {:#x}, which doesn't start in "
                "IMEM range (base {:#x}) or DMEM range "
                "(base {:#x}).".format(seg_lma, imem_lma, dmem_lma)
            )
...
def load_elf(sim: OTBNSim, path: str) -> None:
    ...
    if len(imem_bytes) & 3:
        raise RuntimeError(
            "ELF file at {!r} has imem data of length {}: "
            "not a multiple of 4.".format(path, len(imem_bytes))
        )
