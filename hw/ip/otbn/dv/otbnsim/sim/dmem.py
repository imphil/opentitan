...
class Dmem:
    ...
    def __init__(self) -> None:
        ...
        if dmem_size > 1024 * 1024:
            raise RuntimeError("Implausibly large DMEM size: {}".format(dmem_size))
    ...
    def load_le_words(self, data: bytes) -> None:
        ...
        if len(data) > 32 * len(self.data):
            raise ValueError(
                "Trying to load {} bytes of data, but DMEM "
                "is only {} bytes long.".format(len(data), 32 * len(self.data))
            )
