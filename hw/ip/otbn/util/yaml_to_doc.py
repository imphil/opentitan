...
def render_encoding(
    mnemonic: str, name_to_operand: Dict[str, Operand], encoding: Encoding
) -> str:
...
def render_insn(insn: Insn, heading_level: int) -> str:
    ...
    parts.append("{} {}\n".format("#" * heading_level, insn.mnemonic.upper()))
    ...
    if insn.encoding is not None:
        parts.append(
            render_encoding(insn.mnemonic, insn.name_to_operand, insn.encoding)
        )
    ...
    if insn.decode is not None:
        parts.append(
            ...
            "```python3\n"
            "{}\n"
            "```\n\n".format("#" * (heading_level + 1), insn.decode)
        )
    ...
    if insn.operation is not None:
        parts.append(
            ...
            "```python3\n"
            "{}\n"
            "```\n\n".format("#" * (heading_level + 1), insn.operation)
        )
    return "".join(parts)


def render_insn_group(group: InsnGroup, heading_level: int, out_file: TextIO) -> None:
...
def render_insns(insns: InsnsFile, heading_level: int, out_dir: str) -> None:
