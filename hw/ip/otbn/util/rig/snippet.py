...
class Snippet:
    ...
    def __init__(self, parts: List[Tuple[int, List[ProgInsn]]]):
        assert parts
    ...
    @staticmethod
    def from_json(insns_file: InsnsFile, idx: int, json: object) -> "Snippet":
        ...
        if not isinstance(json, list):
            raise ValueError("Object for snippet {} is not a list.".format(idx))
        ...
        for idx1, part in enumerate(json):
            ...
            if not (isinstance(part, list) and len(part) == 2):
                raise ValueError(
                    "Part {} for snippet {} is not a pair.".format(idx1, idx)
                )
            ...
            if not isinstance(addr, int):
                raise ValueError(
                    "First coordinate of part {} for snippet {} "
                    "is not an integer.".format(idx1, idx)
                )
            if addr < 0:
                raise ValueError(
                    "Address of part {} for snippet {} is {}, "
                    "but should be non-negative.".format(idx1, idx, addr)
                )
            if addr & 3:
                raise ValueError(
                    "Address of part {} for snippet {} is {}, "
                    "but should be 4-byte aligned.".format(idx1, idx, addr)
                )

            if not isinstance(insns_json, list):
                raise ValueError(
                    "Second coordinate of part {} for snippet {} "
                    "is not a list.".format(idx1, idx)
                )

            insns = []
            for insn_idx, insn_json in enumerate(insns_json):
                where = "In snippet {}, part {}, instruction {}".format(
                    idx, idx1, insn_idx
                )
