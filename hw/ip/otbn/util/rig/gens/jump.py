...
class Jump(SnippetGen):
    ...
    def __init__(self, insns_file: InsnsFile) -> None:
        ...
        exp_shape = (
            len(jal.operands) == 2
            and isinstance(jal.operands[0].op_type, RegOperandType)
            and jal.operands[0].op_type.reg_type == "gpr"
            and jal.operands[0].op_type.is_dest()
            and isinstance(jal.operands[1].op_type, ImmOperandType)
            and jal.operands[1].op_type.signed
        )
        if not exp_shape:
            raise RuntimeError(
                "JAL instruction from instructions file is not "
        ...
        exp_shape = (
            len(jalr.operands) == 3
            and isinstance(jalr.operands[0].op_type, RegOperandType)
            and jalr.operands[0].op_type.reg_type == "gpr"
            and jalr.operands[0].op_type.is_dest()
            and isinstance(jalr.operands[1].op_type, RegOperandType)
            and jalr.operands[1].op_type.reg_type == "gpr"
            and not jalr.operands[1].op_type.is_dest()
            and isinstance(jalr.operands[2].op_type, ImmOperandType)
            and jalr.operands[2].op_type.signed
        )
        if not exp_shape:
            raise RuntimeError(
                "JALR instruction from instructions file is "
    ...
    def _pick_jump(
        self, base_addr: int, imm_optype: ImmOperandType, model: Model, program: Program
    ...
    def gen_jal(
        ...
        return self._add_snippet(prog_insn, link_reg_idx, tgt, size, model, program)
