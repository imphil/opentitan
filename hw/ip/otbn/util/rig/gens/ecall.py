...
class ECall(SnippetGen):
    ...
    def __init__(self, insns_file: InsnsFile) -> None:
        ...
        if ecall_insn.operands:
            raise RuntimeError(
                "ECALL instruction in instructions file "
    ...
    def pick_weight(self, size: int, model: Model, program: Program) -> float:
        # Choose small weights when size is large and large ones when it's
        # small.
        assert size > 0
        return 1e-10 if size > 5 else 0.1 if size > 1 else 1e10
