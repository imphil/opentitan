...
def extract_windows(
    reg_byte_width: int, registers: List[HjsonDict]
) -> Dict[str, _Window]:
    ...
    for reg in registers:
        ...
        if window_name in windows:
            raise ValueError(
                "Duplicate window entry with name {!r}.".format(window_name)
            )
...
def get_memory_layout() -> Dict[str, Tuple[int, int]]:
    ...
    for name in xmem_names:
        if name not in windows:
            raise RuntimeError(
                "otbn.hjson doesn't have a window called {}.".format(name)
            )
    if len(windows) != 2:
        raise RuntimeError(
            "Unexpected windows in otbn.hjson: {}".format(
                list(set(windows.keys()) - xmem_names)
            )
        )
