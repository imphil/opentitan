...
class EncSchemeField:
    ...
    def from_yaml(yml: object, what: str) -> "EncSchemeField":
        ...
        if isinstance(yml, dict):
            ...
            raise ValueError(
                "{} is a {}, but should be a "
                "dict, string or integer.".format(what, type(yml).__name__)
            )
        ...
        if raw_value is not None:
            ...
            if bits.width != value.width:
                raise ValueError(
                    "{} has bits that imply a width of {}, but "
                    "a value with width {}.".format(what, bits.width, value.width)
                )
...
class EncSchemeImport:
    ...
    def __init__(self, yml: object, importer_name: str) -> None:
        as_str = check_str(
            yml, "value for import in encoding scheme {!r}".format(importer_name)
        )
        ...
        if not match:
            raise ValueError(
                "Malformed encoding scheme "
                "inheritance by scheme {!r}: {!r}.".format(importer_name, as_str)
            )
        ...
        if match.group(2) is not None:
            ...
            for arg in args:
                ...
                if field_name in self.settings:
                    raise ValueError(
                        "{}, found multiple arguments assigning "
                        "values to the field {!r}.".format(when, field_name)
                    )
    ...
    def apply_settings(self, esf: "EncSchemeFields", what: str) -> "EncSchemeFields":
        ...
        for name, literal in self.settings.items():
            ...
            if old_field is None:
                raise ValueError(
                    "{} sets unknown field {!r} from {!r}.".format(
                        what, name, self.parent
                    )
                )
...
class EncSchemeFields:
    ...
    def __init__(
        self, fields: Dict[str, EncSchemeField], op_fields: Set[str], mask: int
    ) -> None:
        self.fields = fields
    ...
    def from_yaml(yml: object, name: str) -> "EncSchemeFields":
        if not isinstance(yml, dict):
            raise ValueError(
                "fields for encoding scheme {!r} should be a "
                "dict, but we saw a {}.".format(name, type(yml).__name__)
            )
        ...
        for name, field in right.fields.items():
            ...
            if overlap:
                raise ValueError(
                    ...
                    "intersection {:08x}) {}.".format(
                        self.mask, field.bits.mask, overlap, when
                    )
                )
...
class EncSchemes:
    def __init__(self, yml: object) -> None:
        ...
        for key, val in yml.items():
            if not isinstance(key, str):
                raise ValueError(
                    "{!r} is a bad key for an encoding scheme "
                    "name (should be str, not {}).".format(key, type(key).__name__)
                )
    ...
    def _resolve(self, name: str, user: str, stack: List[str]) -> EncSchemeFields:
        ...
        if name in stack:
            raise RuntimeError(
                "Circular inheritance of encoding "
                "schemes: {}".format(" -> ".join(stack + [name]))
            )
        ...
        if scheme is None:
            raise ValueError(
                "{} requires undefined encoding scheme {!r}.".format(user, name)
            )
        ...
        for pname, pfields in resolved_parents.items():
            when = (
                "merging fields of scheme {} into "
                "already merged fields of {}".format(
        ...
        when = "merging direct fields of scheme {} into fields from parents".format(
            name
        )
    ...
    def resolve(self, name: str, mnemonic: str) -> EncSchemeFields:
        ...
        if missing:
            raise ValueError(
                "Fields for encoding scheme {} miss some bits "
                "(mask: {:#08x})".format(name, missing)
            )
