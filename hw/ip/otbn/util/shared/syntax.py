...
class SyntaxToken:
    ...
    def render(self, op_vals: Dict[str, int], operands: Dict[str, Operand]) -> str:
...
class SyntaxHunk:
    ...
    def render(self, op_vals: Dict[str, int], operands: Dict[str, Operand]) -> str:
        ...
        return "".join(token.render(op_vals, operands) for token in self.tokens)
...
class InsnSyntax:
    ...
    def __init__(
        self, hunks: List[SyntaxHunk], op_list: List[str], op_set: Set[str]
    ) -> None:
        self.hunks = hunks
        self.op_list = op_list
        self.op_set = op_set
    ...
    def from_yaml(mnemonic: str, raw: str) -> "InsnSyntax":
        ...
        for after_left in by_left[1:]:
            ...
            if len(split) != 2:
                raise ValueError(
                    "Unbalanced or nested [] in instruction "
                    "syntax for {!r}.".format(mnemonic)
                )
        ...
        for optional, raw in parts:
            if raw:
                ...
                raise ValueError(
                    "Empty [] in instruction syntax for {!r}.".format(mnemonic)
                )
        ...
        if len(op_list) != len(op_set):
            raise ValueError(
                "Instruction syntax for {!r} is not "
                "linear in its operands.".format(mnemonic)
            )
