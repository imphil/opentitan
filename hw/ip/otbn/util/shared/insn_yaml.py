...
from .yaml_parse_helpers import (
    check_keys,
    check_str,
    check_bool,
    check_int,
...
class Insn:
    def __init__(self, yml: object, encoding_schemes: Optional[EncSchemes]) -> None:
        yd = check_keys(
            yml,
            "instruction",
            ["mnemonic", "operands"],
        ...
        self.name_to_operand = index_list(
            "operands for " + what, self.operands, lambda op: op.name
        )
        ...
        self.glued_ops = check_bool(
            yd.get("glued-ops", False), "glued-ops flag for " + what
        )
        ...
        if self.python_pseudo_op and self.encoding is not None:
            raise ValueError(
                "{} specifies an encoding and also sets "
        ...
        if lpo is None:
            ...
            if self.python_pseudo_op:
                raise ValueError(
                    "{} specifies both python-pseudo-op and "
                    "literal-pseudo-op.".format(what)
                )
            if self.encoding is not None:
                raise ValueError(
                    "{} specifies both an encoding and "
                    "literal-pseudo-op.".format(what)
                )
        ...
        self.cycles = check_int(yd.get("cycles", 1), "cycles field for " + what)
        if self.cycles <= 0:
            raise ValueError("cycles field for {} is not positive.".format(what))

    def enc_vals_to_op_vals(
        self, cur_pc: int, enc_vals: Dict[str, int]
    ) -> Dict[str, int]:
    ...
    def disassemble(self, op_vals: Dict[str, int], mnem_width: int) -> str:
...
class InsnGroup:
    def __init__(
        self, path: str, encoding_schemes: Optional[EncSchemes], yml: object
    ) -> None:
        ...
        try:
            self.insns = [
                Insn(i, encoding_schemes) for i in check_list(insns_yaml, insns_what)
            ]
...
class InsnGroups:
    def __init__(
        ...
        self.groups = [
            InsnGroup(path, encoding_schemes, y) for y in check_list(yml, "insn-groups")
        ]
        if not self.groups:
            raise ValueError(
                "Empty list of instruction groups: "
...
class InsnsFile:
    def __init__(self, path: str, yml: object) -> None:
        ...
        self.groups = InsnGroups(path, self.encoding_schemes, yd["insn-groups"])
        ...
        self.mnemonic_to_insn = index_list(
            "insns", self.insns, lambda insn: insn.mnemonic.lower()
        )
        ...
        if ambiguous_encodings:
            ...
            for mnem0, mnem1, bits in ambiguous_encodings:
                ambiguity_msgs.append(
                    "{!r} and {!r} "
                    "both match bit pattern {:#010x}".format(mnem0, mnem1, bits)
                )
            raise ValueError(
                "Ambiguous instruction encodings: " + ", ".join(ambiguity_msgs)
            )
...
def load_file(path: str) -> InsnsFile:
    ...
    try:
        ...
        raise RuntimeError(
            "Invalid schema in YAML file at {!r}: {}".format(path, err)
        ) from None
