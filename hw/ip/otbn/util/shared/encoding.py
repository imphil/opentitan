...
class EncodingField:
    ...
    def __init__(
        self, value: Union[BoolLiteral, str], scheme_field: EncSchemeField
    ) -> None:
        self.value = value
    ...
    @staticmethod
    def from_yaml(
        as_str: str, scheme_field: EncSchemeField, what: str
    ) -> "EncodingField":
        ...
        if re.match(r"b[01x_]+$", as_str):
            ...
            if scheme_field.bits.width != value.width:
                raise ValueError(
                    "{} is mapped to a literal value with width "
                    "{}, but the encoding schema field has "
                    "width {}.".format(what, value.width, scheme_field.bits.width)
                )
        else:
            value = as_str
    ...
    def __init__(self, yml: object, schemes: EncSchemes, mnemonic: str):
        ...
        for field_name, scheme_field in scheme_fields.fields.items():
            if scheme_field.value is not None:
                ...
                field_what = (
                    "value for {} field in "
                    "encoding for instruction {!r}".format(field_name, mnemonic)
                )
                ef_val = check_str(ydm[field_name], field_what)
                field = EncodingField.from_yaml(
                    ef_val, scheme_fields.fields[field_name], field_what
                )
                    ...
                    if other_field_name is not None:
                        raise ValueError(
                            ...
                            "{!r}: used in fields {!r} and {!r}.".format(
                                field.value, mnemonic, other_field_name, field_name
                            )
                        )
    ...
    def assemble(self, op_to_idx: Dict[str, int]) -> int:
        ...
        for field_name, field in self.fields.items():
            ...
            if field_val >> field.scheme_field.bits.width:
                raise ValueError(
                    "operand field {} has a width of {}, "
                    "so can't represent the value {:#x}.".format(
                        field.value, field.scheme_field.bits.width, field_val
                    )
                )
