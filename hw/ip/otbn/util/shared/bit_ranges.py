...
class BitRanges:
    ...
    def from_yaml(as_string: str, what: str) -> "BitRanges":
        ...
        for rng in as_string.split(","):
            ...
            if match is None:
                raise ValueError(
                    "Range {!r} in bits for {} is malformed.".format(rng, what)
                )
            ...
            if msb < lsb:
                raise ValueError(
                    "Range {!r} in bits for {} has msb < lsb.".format(rng, what)
                )

            if msb >= 32:
                raise ValueError(
                    "Range {!r} in bits for {} has msb >= 32.".format(rng, what)
                )
        ...
        if overlaps:
            raise ValueError(
                "Bits for {} have overlapping ranges "
                "(mask: {:#08x})".format(what, overlaps)
            )
