...
def check_bool(obj: object, what: str) -> bool:
    ...
    if obj is not True and obj is not False:
        raise ValueError(
            "{} is of type {}, not a bool.".format(what, type(obj).__name__)
        )
    return obj
...
def index_list(
    what: str, objs: Sequence[T], get_key: Callable[[T], str]
) -> Dict[str, T]:
    ret = {}
    for obj in objs:
        key = get_key(obj)
        if key in ret:
            raise ValueError("Duplicate object with key {} in {}.".format(key, what))
        ret[key] = obj
    return ret


def get_optional_str(data: Dict[str, object], key: str, what: str) -> Optional[str]:
...
def load_yaml(path: str, what: Optional[str]) -> object:
    ...
    try:
        ...
        raise RuntimeError(
            "Cannot find YAML file{} at {!r}.".format(for_msg, path)
        ) from None
    except yaml.YAMLError as err:
        raise RuntimeError(
            "Failed to parse YAML file{} at {!r}: {}".format(for_msg, path, err)
        ) from None
