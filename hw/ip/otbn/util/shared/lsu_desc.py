        ...
        if isinstance(target, str):
            ...
            if not isinstance(target, list):
                raise ValueError(
                    ...
                    "list of strings, but actually has type {}.".format(
        ...
        if lsu_type.startswith("mem-"):
            if "bytes" not in yd:
                raise ValueError(
                    ...
                    "a bytes field (how many bytes does this touch?)".format(what)
                )
            idx_width = check_int(yd["bytes"], "bytes field for " + what)
        else:
            if "bytes" in yd:
                raise ValueError(
                    "{} isn't a memory operation, so cannot have "
                    "a bytes field.".format(what)
                )
            idx_width = 1
