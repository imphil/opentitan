...
def _prim_cores(cores, prim_name=None):
    ...
    def _filter_primitives(core):
        ...
        if (
            vlnv["vendor"] == "lowrisc"
            and vlnv["library"].startswith("prim_")
            and (prim_name is None or vlnv["name"] == prim_name)
        ):
...
def _parse_module_header(generic_impl_filepath, module_name):
    ...
    RE_MODULE_HEADER = (
        ...
        + module_name
        + r"\s*(?P<package_import_declaration>(?:import\s+[^;]+;)+)?"  # module_identifier  # package_import_declaration (optional, skipped)
    ...
    if not matches:
        raise ValueError(
            "Unable to extract module header from %s." % (generic_impl_filepath,)
        )

    parameter_port_list = matches.group("parameter_port_list") or ""
    return {
        "module_header": matches.group("module_header").strip(),
        "package_import_declaration": matches.group("package_import_declaration") or "",
        "parameter_port_list": parameter_port_list,
        "ports": matches.group("ports").strip() or "",
        "parameters": _parse_parameter_port_list(parameter_port_list),
    }
...
def _check_gapi(gapi):
    if "cores" not in gapi:
        print(
            "Key 'cores' not found in GAPI structure. "
...
def _generate_prim_pkg(gapi):
    ...
    prim_pkg_sv_tpl_filepath = os.path.join(
        os.path.dirname(__file__), "primgen", "prim_pkg.sv.tpl"
    )
    prim_pkg_sv_tpl = Template(filename=prim_pkg_sv_tpl_filepath)

    prim_pkg_sv = prim_pkg_sv_tpl.render(encoding="utf-8", techlib_enums=techlib_enums)
    with open("prim_pkg.sv", "w") as f:
        f.write(prim_pkg_sv)

    # Copy prim_pkg.core (no changes needed)
    prim_pkg_core_src = os.path.join(
        os.path.dirname(__file__), "primgen", "prim_pkg.core.tpl"
    )
...
def _create_instances(prim_name, techlibs, parameters):
    ...
    techlibs_wo_generic = [techlib for techlib in techlibs if techlib != "generic"]
    ...
    for pos, techlib in enumerate(techlibs_generic_last):
        ...
        s += (
            "begin : gen_{techlib}\n"
            + _instance_sv(prim_name, techlib, parameters)
            + "end"
        )
...
def _generate_abstract_impl(gapi):
    ...
    if "generic" not in techlibs:
        raise ValueError(
            "Techlib generic is required, but not found for "
            "primitive %s." % prim_name
        )
    print("Implementations for primitive %s: %s" % (prim_name, ", ".join(techlibs)))
    ...
    top_module_filename = _top_module_file(generic_core["files"], generic_module_name)
    top_module_file = os.path.join(generic_core["core_root"], top_module_filename)
    ...
    abstract_prim_sv_tpl_filepath = os.path.join(
        os.path.dirname(__file__), "primgen", "abstract_prim.sv.tpl"
    )
    ...
    dependencies += [_core_info_for_techlib(prim_cores, t)[0] for t in techlibs]
    abstract_prim_core = {
