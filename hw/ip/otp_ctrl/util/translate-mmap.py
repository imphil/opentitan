...
def validate(config):
    ...
    for part in config["partitions"]:
        ...
        log.info(
            "Partition {} at offset {} with size {}".format(
        ...
        if part["key_sel"] not in ["NoKey", "Secret0Key", "Secret1Key", "Secret2Key"]:
            log.error("Invalid key sel {}".format(part["key_sel"]))
            exit(1)

        if check_bool(part["secret"]) and part["key_sel"] == "NoKey":
            log.error("A secret partition needs a key select value other than NoKey")
            exit(1)
        ...
        if not part["sw_digest"] and not part["hw_digest"]:
            if (
                part["write_lock"].lower() == "digest"
                or part["read_lock"].lower() == "digest"
            ):
                log.error(
        ...
        for item in part["items"]:
            ...
            log.info(
                "> Item {} at offset {} with size {}".format(
        ...
        if part["sw_digest"] or part["hw_digest"]:
            part["items"].append(
                {
                    "name": part["name"] + DIGEST_SUFFIX,
                    "size": DIGEST_SIZE,
                    "offset": check_int(part["offset"])
                    + check_int(part["size"])
                    - DIGEST_SIZE,
                    "isdigest": "True",
            ...
            log.info(
                "> Adding digest {} at offset {} with size {}".format(
    ...
    otp_size = check_int(config["otp"]["depth"]) * check_int(config["otp"]["width"])
...
def create_partitions_table(config):
    ...
    for part in config["partitions"]:
        ...
        is_buffered = "yes" if part["variant"] in ["Buffered", "LifeCycle"] else "no"
        wr_lockable = "no"
        ...
        row = [part["name"], is_secret, is_buffered, wr_lockable, rd_lockable, desc]
        table.append(row)
    ...
    for k, part in enumerate(config["partitions"]):
        for j, item in enumerate(part["items"]):
            ...
            if check_bool(item["isdigest"]):
                granule = "64bit"
                name = "[{}](#Reg_{}_0)".format(item["name"], item["name"].lower())
            else:
                name = item["name"]
            ...
            row.extend(
                [name, "0x{:03X}".format(check_int(item["offset"])), str(item["size"])]
            )
    ...
    for part in config["partitions"]:
        if check_bool(part["hw_digest"]) or check_bool(part["sw_digest"]):
            ...
            for item in part["items"]:
                ...
                log.error("Partition with digest does not contain a digest item")
                exit(1)
    ...
    with open(MMAP_DEFINITION_FILE, "r") as infile:
        ...
        for template in TEMPLATES:
            with open(template, "r") as tplfile:
                ...
                with open(
                    Path(template).parent.joinpath(Path(template).stem), "w"
                ) as outfile:
