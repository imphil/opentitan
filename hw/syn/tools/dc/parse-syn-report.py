...
def _match_fp_number(full_file, patterns):
    ...
    for key, pattern in patterns:
        ...
        if not matches:
            errs.append("Pattern {!r} of key {!r} not found".format(pattern, key))
            continue
...
def _extract_messages(full_file, results, key, args):
    ...
    for severity, pattern in err_warn_patterns:
        results["messages"][severity] += re.findall(
            pattern, full_file, flags=re.MULTILINE
        )
    return results
...
def _extract_area(full_file, results, key, args):
    ...
    """
    patterns = [
        ("comb", r"^Combinational area: \s* (\d+\.\d+)"),
    ...
    if args.depth == 2:
        pattern = (
            r"^args.dut/([\.0-9A-Za-z_\[\]]+){1}" + r"(?:(?:/[\.0-9A-Za-z_\[\]]+)*)"
        )
    elif args.depth > 2:
    ...
    try:
        for item in matches:
            ...
            results[key]["instances"][item[0]]["total"] += (
    ...
    if _rel_err(reg_check, results[key]["reg"]) > CROSSCHECK_REL_TOL:
        results["messages"]["flow_errors"] += [
            "Reporting error: reg_check (%e) != (%e)" % (reg_check, results[key]["reg"])
        ]
...
def _extract_clocks(full_file, results, key, args):
    ...
    """
    clocks = re.findall(
        r"^(.+)\s+(\d+\.?\d*)\s+\{\d+.?\d* \d+.?\d*\}\s+", full_file, flags=re.MULTILINE
    )
    try:
        # get clock period
        for k, c in enumerate(clocks):
            if c[0].strip() not in results[key]:
                results[key].update(
                    {c[0].strip(): {"tns": 0.0, "wns": 0.0, "period": float(c[1])}}
                )
    except ValueError as err:
        results["messages"]["flow_errors"] += ["ValueError: %s" % err]
...
def _extract_timing(full_file, results, key, args):
    ...
    """
    groups = re.findall(r"^  Path Group:\s(.+)\s", full_file, flags=re.MULTILINE)

    slack = re.findall(
        r"^  slack \(.+\) \s*(" + FP_NUMBER + ")", full_file, flags=re.MULTILINE
    )
    try:
        ...
        for k, g in enumerate(groups):
            if g.strip() not in results[key]:
                results[key].update(
                    {g.strip(): {"tns": 0.0, "wns": 0.0, "period": float("nan")}}
                )
            ...
            if match:
                if len(match[0]) == 2:
                    if match[0][1].strip() in units:
                        results[key][subkey] = (
                            float(match[0][0]) * units[match[0][1].strip()]
                        )
        except ValueError as err:
            results["messages"]["flow_errors"] += ["ValueError: %s" % err]
...
def _extract_units(full_file, results, key, args):
    ...
    patterns = [
        (
            "voltage",
            r"^    Voltage Units = (\d+\.?\d*)(nV|uV|mV|V)",
            {"nV": 1e-9, "uV": 1e-6, "mV": 1e-3, "V": 1e0},
        ),
        (
            "capacitance",
            r"^    Capacitance Units = (\d+\.?\d*)(ff|pf|nf|uf)",
            {"ff": 1e-15, "pf": 1e-12, "nf": 1e-9, "uf": 1e-6},
        ),
        (
            "time",
            r"^    Time Units = (\d+\.?\d*)(ps|ns|us|ms)",
            {"ps": 1e-12, "ns": 1e-9, "us": 1e-6, "ms": 1e-3},
        ),
        (
            "dynamic",
            r"^    Dynamic Power Units = (\d+\.?\d*)(pW|nW|uW|mW|W)",
            {"pW": 1e-12, "nW": 1e-9, "uW": 1e-6, "mW": 1e-3, "W": 1e0},
        ),
        (
            "static",
            r"^    Leakage Power Units = (\d+\.?\d*)(pW|nW|uW|mW|W)",
            {"pW": 1e-12, "nW": 1e-9, "uW": 1e-6, "mW": 1e-3, "W": 1e0},
        ),
    ]
...
def get_results(args):
    ...
    # flow messages
    results = _parse_file(
        args.logpath, "synthesis.log", results, _extract_messages, "flow", args
    )
    ...
    results = _parse_file(
        args.reppath, "gate_equiv.rpt", results, _extract_gate_equiv, "area", args
    )
    # area
    results = _parse_file(
        args.reppath, "area.rpt", results, _extract_area, "area", args
    )
    # clocks. this complements the timing report later below
    results = _parse_file(
        args.reppath, "clocks.rpt", results, _extract_clocks, "timing", args
    )
    # timing
    results = _parse_file(
        args.reppath, "timing.rpt", results, _extract_timing, "timing", args
    )
    # power
    results = _parse_file(
        args.reppath, "power.rpt", results, _extract_power, "power", args
    )
    # units
    results = _parse_file(
        args.reppath, "power.rpt", results, _extract_units, "units", args
    )
...
def main():
    ...
    parser.add_argument(
        "--depth", type=int, default=1, help="""Area Report with hierarchical depth"""
    )
