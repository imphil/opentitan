...
def main():
    ...
    parser.add_argument(
        "-t", "--title", type=str, default="AT Plot", help="Title of AT plot."
    )
    parser.add_argument(
    ...
    for basename in args.result_series:
        ...
        for rpt_area in report_filebase.parent.rglob(
            report_filebase.name + "_*_area.rpt"
        ):
            ...
            try:
                with open(rpt_area, "r") as f:
                    full_file = f.read()
                    tmp_area = re.findall(
                        r"^Total cell area:.*", full_file, flags=re.MULTILINE
                    )
                    if tmp_area:
                        ...
                        print(
                            "Error, could not find total cell area in %s"
                            % (report_area)
                        )
                        sys.exit(1)
            ...
            try:
                ...
                with open(rpt_timing, "r") as f:
                    full_file = f.read()
                    tmp_slack = re.findall(
                        r"^  slack \(.*", full_file, flags=re.MULTILINE
                    )
                    if tmp_slack:
        ...
        if np.any(results):
            ...
            for k in range(len(results)):
                print(
                    "%.2f, %.2f, %.2f" % (results[k][0], results[k][2], results[k][1])
                )
            if args.semilogy:
                plt.semilogy(
                    results[:, 2],
