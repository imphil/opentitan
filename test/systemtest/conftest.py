...
def localconf():
    ...
    if "OPENTITAN_TEST_LOCALCONF" in os.environ:
        ...
        log.info(
            "Attempting to use configuration file set in " "OPENTITAN_TEST_LOCALCONF."
        )
    else:
        XDG_CONFIG_HOME = Path(
            os.getenv("XDG_CONFIG_HOME", os.path.join(os.environ["HOME"], ".config"))
        )
...
def bin_dir(topsrcdir):
    ...
    if "BIN_DIR" in os.environ:
        ...
        log.info(
            "Using build outputs from environment variable BIN_DIR={}".format(
                str(bin_dir)
            )
        )
    else:
        ...
        log.info(
            "Using build outputs from $REPO_TOP/build-bin ({})".format(str(bin_dir))
        )
