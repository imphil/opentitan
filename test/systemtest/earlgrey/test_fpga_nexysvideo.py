...
def uart_persistent(localconf_nexysvideo, tmp_path_factory):
    ...
    log.debug("Opening UART on device {} ({} baud)".format(uart_device, uart_speed))
    with utils.LoggingSerial(
        uart_device,
        uart_speed,
        timeout=1,
...
def uart(uart_persistent, request):
    ...
    uart_persistent.log_add_marker(
...
def nexysvideo_earlgrey(tmp_path_factory, topsrcdir, bin_dir, localconf_nexysvideo):
    ...
    bitstream = (
        bin_dir / "hw/top_earlgrey/lowrisc_systems_top_earlgrey_nexysvideo_0.1.bit"
    )
    ...
    log.debug("Flashing Nexys Video board with bitstream {}".format(str(bitstream)))
...
def app_selfchecking_bin(request, bin_dir):
    ...
    if "targets" in app_config and "fpga_nexysvideo" not in app_config["targets"]:
...
def test_apps_selfchecking(
    ...
    assert uart.find_in_output(
        bootmsg_exp, timeout=10
    ), "End-of-bootrom string not found."

    log.debug("Waiting for pass string from device test")

    result_match = uart.find_in_output(re.compile(br"^(PASS|FAIL)!$"), timeout=60)
