...
def get_available_toolchain_info(version, kind):
    ...
    for asset in release_info["assets"]:
        if asset["name"].startswith(ASSET_PREFIXES[kind]) and asset["name"].endswith(
            ASSET_SUFFIX
        ):
            return {
    ...
    log.error(
        "No available downloads found for %s toolchain version: %s",
        kind,
...
def main():
    ...
    if args.update and unpack_dir.is_dir():
        ...
        version_matches = (
            available_toolchain["version"] == installed_toolchain["version"]
        )
        kind_matches = available_toolchain["kind"] == installed_toolchain["kind"]

        if (
            installed_toolchain["kind"] != "unknown"
            and version_matches
            and kind_matches
        ):
            log.info(
