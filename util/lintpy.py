...
def show_and_exit(clitool, packages):
    ...
    ver = subprocess.check_output(
        ["git", "describe", "--always", "--dirty", "--broken"], universal_newlines=True
    ).strip()
    if not ver:
...
def run_linter(tool, dofix, verbose, files):
    ...
    subprocess.check_call(
        fix + files, stderr=subprocess.STDOUT, stdout=subprocess.DEVNULL
    )
    return (True, False)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Only check files staged for commit rather than"
