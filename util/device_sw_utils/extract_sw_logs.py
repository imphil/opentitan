...
def get_addr_strings(ro_contents):
    ...
    for ro_content in ro_contents:
        ...
        while str_start < size:
            ...
            string = cleanup_newlines(
                data[str_start:str_end].decode("utf-8", errors="replace")
            )
            if addr in result:
    ...
    with open(elf_file, "rb") as f:
        ...
        for ro_section in ro_sections:
            ...
            if section:
                ...
                print("Error: {} section not found in {}".format(ro_section, elf_file))
                sys.exit(1)
        ...
        if section:
            ...
            print(
                "Error: {} section not found in {}".format(
...
def main():
    ...
    parser.add_argument(
        "--name", "-n", required=True, help="Type of the SW elf being processed."
    )
    ...
    rodata, result = extract_sw_logs(
        args.elf_file, args.logs_fields_section, ro_sections
    )
