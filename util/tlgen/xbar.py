...
class Xbar:
    ...
    def __init__(self):
        ...
        self.ip_path = (
            ""  # additional path to generated rtl/dv folders: outdir/ip_path/rtl
        )
    ...
    def __repr__(self):
        out = "<Xbar(%s) #nodes:%d clock:%s" % (self.name, len(self.nodes), self.clock)
        out += " #edges:%d>\n" % (len(self.edges))
    ...
    def get_edges_from_node(self, node):  # Node -> Edges
        return [
            edge for edge in self.edges if node.name in (edge.us.name, edge.ns.name)
        ]
    ...
    def get_downstream_device(self, node):  # Node -> Node
        ...
        if len(node.ds) == 0:
            log.error(
                ...
                % (
                    node.name,
                    " ".join(map(repr, node.us)),
                    " ".join(map(repr, node.ds)),
                )
            )
    ...
    def get_devices_from_host(self, host):  # Node -> Nodes
        devices = list(
            map(self.get_downstream_device_from_edge, self.get_s1n_if_exist(host).ds)
        )
