...
""",
    "required": {
        "base_addr": ["d", "Base address of the device. It is required for the device"],
...
""",
    ...
    "optional": {
        ...
        "pipeline_byp": [
            "pb",
            "Pipeline bypass. If true, " "request/response are not latched",
        ],
...
def check_keys(obj, control, prefix=""):
    ...
    for k, v in obj.items():
        ...
        if checker[0] == "lg":
            # List of subgroup
            error += sum(
                map(partial(check_keys, control=checker[1], prefix=prefix_name), obj[k])
            )
        elif checker[0] == "g":
...
def validate(obj: OrderedDict) -> Xbar:  # OrderedDict -> Xbar
    ...
    for nodeobj in obj["nodes"]:
        ...
        if node.node_type in [NodeType.DEVICE, NodeType.HOST] and "pipeline" in nodeobj:
            node.pipeline = (
                True if nodeobj["pipeline"].lower() in ["true", "1"] else False
            )
        else:
            node.pipeline = False
        if (
            node.node_type in [NodeType.DEVICE, NodeType.HOST]
            and "pipeline_byp" in nodeobj
        ):
            node.pipeline_byp = (
                True if nodeobj["pipeline_byp"].lower() in ["true", "1"] else False
            )
        else:
