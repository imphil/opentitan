...
def autoconnect_xbar(topcfg: OrderedDict, xbar: OrderedDict):
    ...
    for port in ports:
        ...
        ips = [
            x for x in topcfg["module"] + topcfg["memory"] if x["name"] == port["name"]
        ]
        ...
        # get the port name
        def get_signame(
            ip: OrderedDict, package: str, struct: str, act: str
        ) -> OrderedDict:
            ims = [
                x
                for x in ip["inter_signal_list"]
                if (x["package"] == package if "package" in x else False)
                and x["struct"] == struct
                and x["act"] == act
            ]
            return ims
    ...
    for req, rsps in topcfg["inter_module"]["connect"].items():
        ...
        for i, rsp in enumerate(rsps):
            ...
            if req_struct["struct"] == "logic":
                assert (
                    req_struct["type"] != "req_rsp"
                ), "logic signal cannot have req_rsp type"

            if rsp_len != 1:
                log.warning("{req}[{i}] -> {rsp}".format(req=req, i=i, rsp=rsp))
            else:
...
def check_intermodule_field(obj: OrderedDict, prefix: str = "") -> int:
    ...
    if obj["type"] not in IM_TYPES:
        log.error(
            "{prefix} Inter_signal {name} "
            "type {type} is incorrect.".format(
                prefix=prefix, name=obj["name"], type=obj["type"]
            )
        )
        error += 1
    ...
    if obj["act"] not in IM_ACTS:
        log.error(
            "{prefix} Inter_signal {name} "
            "act {act} is incorrect.".format(
                prefix=prefix, name=obj["name"], act=obj["act"]
            )
        )
        error += 1
    ...
    if error == 0:
        if obj["act"] not in IM_VALID_TYPEACT[obj["type"]]:
            log.error(
                "{type} and {act} of {name} are not a valid pair."
                "".format(type=obj["type"], act=obj["act"], name=obj["name"])
            )
            error += 1
    # Check 'width' field
    width = 1
    if "width" not in obj:
        ...
        if err:
            log.error(
                "{prefix} Inter-module {inst}.{sig} 'width' "
                "should be int type.".format(
                    prefix=prefix, inst=obj["inst_name"], sig=obj["name"]
                )
            )
            error += 1
        else:
...
def find_otherside_modules(topcfg: OrderedDict, m, s) -> List[Tuple[str, str, str]]:
    ...
    log.error(
        "`find_otherside_modules()`: " "No such signal {}.{} exists.".format(m, s)
    )
    return []
...
def check_intermodule(topcfg: Dict, prefix: str) -> int:
    ...
    for req, rsps in topcfg["inter_module"]["connect"].items():
        ...
        if req_struct["width"] != rsps_width:
            log.error(
                "Request {} width is not matched with total responses width {}".format(
                    req_struct["width"], rsps_width
                )
            )
            error += 1
