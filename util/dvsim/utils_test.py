...
def test_subst_wildcards():
    ...
    assert (
        subst_wildcards(
            "{a}, {b}, {c}, {d}", {"a": "a", "b": True, "c": 42, "d": ["{b}", 10]}
        )
        == "a, 1, 42, 1 10"
    )
    ...
    assert (
        subst_wildcards("{a} {b}", {"a": "aye", "b": "bee"}, ignored_wildcards=["a"])
        == "{a} bee"
    )
    assert (
        subst_wildcards("{a} {b}", {"b": "bee"}, ignored_wildcards=["a"]) == "{a} bee"
    )
    ...
    # ignore_error=True.
    assert (
        subst_wildcards("{biggles} {b}", {"b": "bee"}, ignore_error=True)
        == "{biggles} bee"
    )
    ...
    assert (
        subst_wildcards(
            "{var}",
            {"var": "{{foo}_xyz_{bar}}", "foo": "p", "bar": "q", "p_xyz_q": "baz"},
        )
        == "baz"
    )
