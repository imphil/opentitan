...
class FlowCfg:
    ...
    def _expand(self):
        ...
        self.__dict__ = find_and_substitute_wildcards(
            self.__dict__, self.__dict__, self.ignored_wildcards, ignore_error=partial
        )
    ...
    def create_instance(self, mk_config, flow_cfg_file):
        ...
        if type(self) is not type(new_instance):
            log.error(
                "{}: Loading child configuration at {!r}, but the "
    ...
    def _conv_inline_cfg_to_hjson(self, idict):
        ...
        temp_cfg_file = (
            self.scratch_root + "/." + self.branch + "__" + name + "_cfg.hjson"
        )

        # Create the file and dump the dict as hjson
        log.log(
            VERBOSE, 'Dumping inline cfg "%s" in hjson to:\n%s', name, temp_cfg_file
        )
        try:
        ...
        if hasattr(self, "overrides"):
            ...
            for item in overrides:
                if type(item) is dict and set(item.keys()) == {"name", "value"}:
                    ...
                    log.error(
                        '"overrides" is a list of dicts with {"name": <name>, '
    ...
    def _do_override(self, ov_name, ov_value):
        ...
        if hasattr(self, ov_name):
            ...
            if type(orig_value) == type(ov_value):
                ...
                log.error(
                    'The type of override value "%s" for "%s" mismatches '
    ...
    def prune_selected_cfgs(self):
        ...
        if not self.is_primary_cfg:
            log.error(
                "The configuration file at {!r} is not a primary config, "
                "but --select-cfgs was passed on the command line.".format(
                    self.flow_cfg_file
                )
            )
            sys.exit(1)
    ...
    def gen_email_html_summary(self):
        ...
        results_html = md_results_to_html(
            self.results_title, self.css_file, gen_results
        )
    ...
    def _publish_results(self):
        ...
        if cmd_output.returncode == 0:
            try:
                ...
                log.error("%s: '%s' Timestamp conversion value error raised!", e)
                old_results_ts = ""

            # If the timestamp conversion failed - then create a dummy one with
            # yesterday's date.
            if old_results_ts == "":
                log.log(VERBOSE, "Creating dummy timestamp with yesterday's date")
            ...
            if cmd_output.returncode != 0:
                log.error(
                    'Failed to mv old results page "%s" to "%s"!',
                    self.results_server_dir,
                    old_results_dir,
                )
        ...
        log.log(VERBOSE, cmd)
        cmd_output = subprocess.run(
            args=cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL
        )
        log.log(VERBOSE, cmd_output.stdout.decode("utf-8"))
        if cmd_output.returncode == 0:
        ...
        # Start pruning
        log.log(
            VERBOSE, "Pruning %s area to limit last 7 results", self.results_server_path
        )
        ...
        try:
            cmd_output = subprocess.run(
                args=cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
            )
            log.log(VERBOSE, cmd_output.stdout.decode("utf-8"))
        except Exception as e:
    ...
    def publish_results_summary(self):
        ...
        f.write(
            md_results_to_html(
                self.results_title, self.css_file, self.results_summary_md
            )
        )
        f.close()
        rm_cmd = "/bin/rm -rf " + results_html_file + "; "
