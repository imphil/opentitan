...
def _load_cfg(path, initial_values):
    ...
    if flow is None:
        raise RuntimeError(
            '{!r}: No value for the "flow" key. Are you sure '
            "this is a dvsim configuration file?".format(path)
        )

    classes = [LintCfg.LintCfg, SynCfg.SynCfg, FpvCfg.FpvCfg, SimCfg.SimCfg]
    found_cls = None
    known_types = []
    for cls in classes:
    ...
    if found_cls is None:
        raise RuntimeError(
            ...
            "this is not a known flow (known: {}).".format(
                path, flow, ", ".join(known_types)
            )
        )
...
def _make_child_cfg(path, args, initial_values):
    ...
    if "use_cfgs" in hjson_data:
        raise RuntimeError(
            "{}: Configuration file has use_cfgs, but is "
            "itself included from another configuration.".format(path)
        )
