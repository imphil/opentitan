...
def main():
    ...
    parser.add_argument(
        "--doc", action="store_true", help="Output source file documentation (gfm)"
    )
    ...
    if dirspec is None:
        if args.outdir is not None:
            log.error(
                "The {} format expects an output file, "
        ...
        if args.outfile is not sys.stdout:
            log.error(
                "The {} format expects an output directory, "
    ...
    try:
        srcfull = infile.read()
        obj = hjson.loads(
            srcfull, use_decimal=True, object_pairs_hook=validate.checking_dict
        )
    except ValueError:
