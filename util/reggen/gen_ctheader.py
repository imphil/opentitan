...
def gen_cdefine_window(outstr, win, comp, regwidth, rnames):
    ...
    genout(
        outstr, "#define " + defname + "(base)\t" + "((base) + " + hex(offset) + ")\n"
    )
    items = int(win["items"])
    ...
    for x in registers:
        ...
        if "sameaddr" in x:
            for sareg in x["sameaddr"]:
                gen_cdefine_register(outstr, sareg, component, regwidth, rnames)
            continue

        if "window" in x:
            gen_cdefine_window(outstr, x["window"], component, regwidth, rnames)
            continue
