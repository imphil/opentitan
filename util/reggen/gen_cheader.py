...
def gen_cdefine_register(outstr, reg, comp, width, rnames, existing_defines):
    ...
    for field in reg["fields"]:
        ...
        if field["bitinfo"][1] == 1:
            # single bit
            genout(
                outstr, gen_define(dname + "_BIT", [], str(fieldlsb), existing_defines)
            )
        else:
            ...
            if field["bitinfo"][1] != width:
                ...
                genout(
                    outstr,
                    gen_define(dname + "_OFFSET", [], str(fieldlsb), existing_defines),
                )
                genout(
                    outstr,
                    gen_define(
...
def gen_cdefines_module_param(outstr, param, module_name, existing_defines):
    ...
    if param_type not in known_types:
        warnings.warn("Cannot generate a module define of type {}".format(param_type))
        return

    genout(outstr, format_comment(first_line(param["desc"])))
    define_name = as_define(module_name + "_PARAM_" + param["name"])
    if param_type == "int":
        define = gen_define(define_name, [], param["default"], existing_defines)
...
def gen_cdefines_module_params(
    outstr, module_data, module_name, register_width, existing_defines
):
    module_params = set()
...
def gen_multireg_field_defines(
    outstr, regname, field, subreg_num, regwidth, existing_defines
):
...
def gen_cdefine_multireg(
    outstr, register, component, regwidth, rnames, existing_defines
):
...
def gen_cdefines_interrupt_field(
    outstr, interrupt, component, regwidth, existing_defines
):
...
def gen_cdefines(regs, outfile, src_lic, src_copy):
    ...
    gen_cdefines_module_params(outstr, regs, component, regwidth, existing_defines)
    ...
    for x in registers:
        ...
        if "sameaddr" in x:
            for sareg in x["sameaddr"]:
                gen_cdefine_register(
                    outstr, sareg, component, regwidth, rnames, existing_defines
                )
            continue

        if "window" in x:
            gen_cdefine_window(
                outstr, x["window"], component, regwidth, rnames, existing_defines
            )
            continue

        if "multireg" in x:
            gen_cdefine_multireg(
                outstr, x, component, regwidth, rnames, existing_defines
            )
            continue
