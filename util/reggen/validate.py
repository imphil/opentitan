...
def check_count(top, mreg, err_prefix):
    ...
    try:
        ...
        return check_int(top["param_list"][index]["default"], err_prefix + " default")
    except ValueError:
        # cannot find entry in the param list
        log.warning(
            err_prefix + " is integer. " + "It is recommended to use Parameter."
        )
...
def check_lp(obj, x, err_prefix):
    ...
    for y in obj[x]:
        error += check_keys(
            y, lp_required, lp_optional, {}, err_prefix + " element " + x
        )
        ...
        if randtype != "none":
            ...
            if y.get("type") is None:
                log.error(
                    err_prefix + " parameter " + y["name"] + " has undefined type. "
            ...
            overrides = [("local", "false"), ("default", ""), ("expose", "false")]
        ...
        if "default" in y:
            if y["type"][:3] == "int":
                default, ierr = check_int(y["default"], err_prefix + " default")
                if ierr:
                    error += 1
                    y["default"] = "1"
...
top_required = {
    ...
    "registers": [
        "l",
        "list of register definition groups and " "offset control groups",
    ],
}
top_optional = {
...
top_added = {
    ...
    "gensize": [
        "pi",
        "address space size needed for registers. "
...
list_optone = {
    ...
    "sameaddr": [
        "l",
        "list of register definition groups " "that share the same offset",
    ],
    "window": [
        "g",
        "group defining an address range "
...
window_optional = {
    ...
    "noalign": [
        "s",
        "Set to True to prevent tool aligning "
...
window_added = {
    ...
    "genoffset": ["pi", "base offset address of the window (aligned for size)"],
...
multireg_added = {
    "genregs": ["l", "generated list of registers with required" " and added keys"]
}
...
field_added = {
    ...
    "genresval": [
        "pi",
        "resval for field constructed by the tool. " "Will be set to 0 for x.",
    ],
...
def validate_fields(
    fields, rname, default_sw, default_hw, full_resval, reg_hwqe, reg_hwre, width, top
):
    error = 0
    bits_used = 0
    gen_resval = 0
    ...
    for field in fields:
            ...
            if fname == "":
                ...
                if fname in fieldnames:
                    ...
                    log.error(
                        ...
                        + " field "
                        + str(fcount)
                        + ": duplicate use of field name "
                        + fname
                    )
                else:
                    fieldnames.append(fname)
                fname = rname + "." + fname
        ck_err = check_keys(field, field_required, field_optional, field_added, fname)
        if ck_err != 0:
            error += ck_err
            continue
        ...
        if field_mask == 0:
            ...
            if field_msb >= width:
                error += 1
                log.error(
                    fname + ": Register not wide enough for bits: " + field["bits"]
                )
            elif reuse_check != 0:
                error += 1
                log.error(fname + ": Defines already defined bits " + hex(reuse_check))
        ...
        if "enum" in field:
            ...
            for enum in field["enum"]:
                eck_err = check_keys(enum, enum_required, [], [], fname + " enum")
                if eck_err != 0:
                    error += eck_err
                    continue
                ename = enum["name"]
...
def validate_reg_defaults(reg, rname):
    ...
    if "hwext" in reg:
        ...
        if ierr:
            ...
            hwext is True
            and default_hw == "hro"
            and (default_sw != "wo" and default_sw != "r0w1c")
        ):
            log.warning(
                rname
    ...
    if "hwqe" in reg:
        ...
        if ierr:
            error += 1
            reg["hwqe"] = "false"
        elif hwqe is False and reg["hwext"] == "true" and reg["swaccess"] != "ro":
            log.warning(
                rname
                + ": hwqe must be true for hwext register. "
                + "Changing it to true."
            )
            reg["hwqe"] = "true"
    elif reg["hwext"] == "true" and reg["swaccess"] != "ro":
        log.warning(
            rname + ": hwqe not provided but must be true for "
    ...
    if "hwre" in reg:
        ...
        if ierr:
            ...
            log.warning(
                rname + ": hwre cannot be used with hwext. " + "Changing it to false."
            )
            reg["hwre"] = "false"
    else:
        reg["hwre"] = "false"
    ...
    if "shadowed" in reg:
        ...
        if rname.lower().endswith("_shadowed"):
            log.warning(
                rname + ": shadowed not provided but must be true for registers with "
...
def validate_register(reg, offset, width, top):
    ...
    if "name" not in reg:
        ...
        if rname in keywords_verilog:
            error += 1
            log.error("Register at +" + hex(offset) + rname + " uses Verilog keywords")
        if rname.lower() in top["genrnames"]:
            error += 1
            log.error("Register at +" + hex(offset) + " duplicate name " + rname)
        else:
            top["genrnames"].append(rname.lower())

    error += check_keys(reg, reg_required, reg_optional, reg_added, rname)

    derr, default_sw, default_hw, full_resval = validate_reg_defaults(reg, rname)
    error += derr
    ...
    log.debug(
        rname + "@" + hex(offset) + " " + str(error) + " errors. Mask " + hex(gen[3])
    )
...
def validate_multi(mreg, offset, addrsep, width, top):
    ...
    error = check_keys(
        mreg, multireg_required, multireg_optional, multireg_added, mrname
    )
    derr, default_sw, default_hw, full_resval = validate_reg_defaults(mreg, mrname)
    error += derr
    ...
    log.debug(
        "Multireg attributes 0x{:x} {} {} {}".format(
...
def make_intr_regs(regs, offset, addrsep, fullwidth):
    ...
    new_reg = make_intr_alert_reg(
        regs, "INTR_STATE", offset, "rw1c", "hrw", "Interrupt State Register"
    )
    iregs.append(new_reg)
    new_reg = make_intr_alert_reg(
        regs, "INTR_ENABLE", offset + addrsep, "rw", "hro", "Interrupt Enable Register"
    )
    iregs.append(new_reg)
    new_reg = make_intr_alert_reg(
        regs, "INTR_TEST", offset + 2 * addrsep, "wo", "hro", "Interrupt Test Register"
    )
    iregs.append(new_reg)
    return iregs, 0
...
def make_alert_regs(regs, offset, addrsep, fullwidth):
    ...
    new_reg = make_intr_alert_reg(
        regs, "ALERT_TEST", offset, "wo", "hro", "Alert Test Register"
    )
...
def validate_window(win, offset, regwidth, top):
    ...
    error += check_keys(win, window_required, window_optional, window_added, name)
    ...
    win["genbyte-write"] = "byte-write" in win and win["byte-write"].lower() == "true"
    ...
    if winsize != po2_size:
        ...
        if not unusual:
            log.warn(
                name + ": Unusual: Size " + str(winitems) + " is not a power of 2."
            )
...
def check_wen_regs(regs):
    ...
    reg_list = [
        (reg["name"].lower(), reg["genresval"], reg["swaccess"], reg["hwaccess"])
        for reg in regs["registers"]
        if "name" in reg and "genresval" in reg and "swaccess" in reg
    ]

    mreg_list = [reg["multireg"] for reg in regs["registers"] if "multireg" in reg]
    ...
    for x in regs["genwennames"]:
        ...
        if reg_list[idx][tuple_swaccess] != "ro" and not reg_list[idx][tuple_rstval]:
            error += 1
            log.error(x + " used as regwen fails requirement to default " + "to 1")
        ...
        if (
            reg_list[idx][tuple_swaccess] == "ro"
            and reg_list[idx][tuple_hwaccess] == "hwo"
        ):
            hw_regwen += 1
...
def validate(regs, **kwargs):
    ...
    if "no_auto_intr_regs" in regs:
        no_auto_intr, err = check_bool(regs["no_auto_intr_regs"], "no_auto_intr_regs")
        if err:
            error += 1
    else:
    ...
    if "no_auto_alert_regs" in regs:
        no_auto_alerts, err = check_bool(
            regs["no_auto_alert_regs"], "no_auto_alert_regs"
        )
        if err:
            error += 1
    else:
    ...
    if regs.setdefault("alert_list", []):
        ...
        if num_alerts != 0:
            ...
            if param:
                ...
                if (
                    param["type"] != "int"
                    or param["default"] != str(num_alerts)
                    or param["local"] != "true"
                ):
    ...
    for x in regs["registers"]:
        ...
        if "reserved" in x:
            nreserved, ierr = check_int(x["reserved"], "Reserved at " + hex(offset))
            if ierr:
                error += 1
            else:
        ...
        if "multireg" in x:
            err, n = validate_multi(x["multireg"], offset, addrsep, fullwidth, regs)
            error += err
            offset += addrsep * n
            continue
    ...
    if error > 0:
        log.error(
            "Register description had " + str(error) + " error" + "s"
            if error > 1
            else ""
        )
