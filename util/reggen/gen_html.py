...
def gen_html_register(outfile, reg, comp, width, rnames, toc, toclvl):
    def gen_merge(outfile, fieldlsb, mergebase, mergeprev, mergedesc):
        ...
        if mergeprev != mergedesc:
            genout(outfile, '<td class="regde">' + mergedesc + ".." + mergeprev[4:])
        else:
    ...
    genout(
        outfile,
        '<table class="regdef" id="Reg_' + rname.lower() + '">\n'
        '<tr><th class="regdef" colspan=5><div>'
        + comp
        + "."
        + rname
        + " @ + "
    ...
    for field in reg["fields"]:
        ...
        if "genbasebits" in reg:
            if ((1 << fieldlsb) & reg["genbasebits"]) == 0:
                ...
                if mergebase >= 0:
                    gen_merge(outfile, fieldlsb, mergebase, mergeprev, mergedesc)
                    mergebase = -1
        ...
        if "desc" in field:
            genout(
                outfile,
                '<td class="regde">' + desc_expand(field["desc"], rnames) + "\n",
            )
        else:
            genout(outfile, "<td>\n")
        ...
        if "enum" in field:
            ...
            for enum in field["enum"]:
                ...
                genout(
                    outfile, "<td>" + desc_expand(enum["desc"], rnames) + "</td></tr>\n"
                )
...
def gen_html_window(outfile, win, comp, regwidth, rnames, toc, toclvl):
    ...
    genout(
        outfile,
        '<table class="regdef" id="Reg_' + wname.lower() + '">\n'
        '<tr><th class="regdef"><div>'
        + comp
        + "."
        + wname
        + " @ + "
        ...
        + " item "
        + win["swaccess"]
        + " window</div><div>Byte writes are "
    ...
    genout(
        outfile,
        '<tr><td class="regde">' + desc_expand(win["desc"], rnames) + "</td></tr>",
    )
    genout(outfile, "</table>\n<br><br>\n")
    if toc is not None:
    ...
    for x in registers:
        ...
        if "sameaddr" in x:
            for sareg in x["sameaddr"]:
                gen_html_register(
                    outfile, sareg, component, regwidth, rnames, toclist, toclevel
                )
            continue

        if "window" in x:
            gen_html_window(
                outfile, x["window"], component, regwidth, rnames, toclist, toclevel
            )
            continue
        ...
        if "multireg" in x:
            for reg in x["multireg"]["genregs"]:
                gen_html_register(
                    outfile, reg, component, regwidth, rnames, toclist, toclevel
                )
            continue

        gen_html_register(outfile, x, component, regwidth, rnames, toclist, toclevel)
    return
