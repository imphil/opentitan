...
def gen_cfg_html(cfgs, outfile):
    ...
    genout(outfile, "the module <b><code>" + cfgs["name"] + "</code></b> has \n")
    genout(outfile, "the following hardware interfaces defined.</p>\n")
    # clocks
    genout(
        outfile,
        "<p><i>Primary Clock:</i> <b><code>"
        + cfgs["clock_primary"]
        + "</code></b></p>\n",
    )
    ...
    genout(
        outfile,
        "<p><i>Bus Device Interface:</i> <b><code>"
        + cfgs["bus_device"]
        + "</code></b></p>\n",
    )
    if "bus_host" in cfgs:
        genout(
            outfile,
            "<p><i>Bus Host Interface:</i> <b><code>"
            + cfgs["bus_host"]
            + "</code></b></p>\n",
        )
    else:
    ...
    # IO
    if (
        "available_input_list" in cfgs
        or "available_output_list" in cfgs
        or "available_inout_list" in cfgs
    ):
    ...
    if "interrupt_list" in cfgs:
        ...
        for x in cfgs["interrupt_list"]:
            genout(
                outfile,
                "<tr><td>" + name_width(x) + "</td><td>" + x["desc"] + "</td></tr>\n",
            )
        genout(outfile, "</table>\n")
    else:
    ...
    if "alert_list" in cfgs:
        ...
        for x in cfgs["alert_list"]:
            genout(
                outfile,
                "<tr><td>" + x["name"] + "</td><td>" + x["desc"] + "</td></tr>\n",
            )
        genout(outfile, "</table>\n")
    else:
