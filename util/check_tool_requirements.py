    ...
    with open(path, "r") as pyfile:
        ...
        if reqs is None:
            log.error(
                ...
                "__TOOL_REQUIREMENTS__.".format(path)
            )
            return None
        ...
        if not isinstance(reqs, dict):
            log.error(
                "The Python file at {} defined "
                "__TOOL_REQUIREMENTS__, but it is not a dict.".format(path)
            )
            return None
...
def pip3_get_version(tool):
    ...
    try:
        ...
        log.error(
            "Failed to get version of {} with pip3: is it installed?".format(tool)
        )
    ...
    log.error('No output line from running {} started with "Version: ".'.format(cmd))
    return None
...
def check_version(requirements, tool_name, getter):
    ...
    if required_version is None:
        log.error(
            "Requirements file does not specify version for {}.".format(tool_name)
        )
        return False
    ...
    if StrictVersion(actual_version) < StrictVersion(required_version):
        log.error(
            "%s is too old: found version %s, need at least %s",
            tool_name,
            actual_version,
            required_version,
        )
        ...
        log.info(
            "Found sufficiently recent version of %s (found %s, need %s)",
            tool_name,
            actual_version,
            required_version,
        )
...
def main():
    ...
    all_good = True
    all_good &= check_version(tool_requirements, "verilator", get_verilator_version)
    all_good &= check_version(
        tool_requirements, "edalize", lambda: pip3_get_version("edalize")
    )

    if not all_good:
        log.error(
            "Tool requirements not fulfilled. " "Please update the tools and retry."
        )
        return 1
